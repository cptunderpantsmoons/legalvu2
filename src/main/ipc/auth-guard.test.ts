...
describe('IPC auth guards', () => {
  ...
  it('AUTH_LOGIN works without auth (no user logged in)', () => {
    ...
    seedTestUser(db, 'test-user-1', 'login@test.com');
    // Set a known password hash via the service
    const registered = authService.register('login@test.com', 'password123', 'Login User');
  ...
  it('CONTRACT_LIST succeeds when authenticated', () => {
    const user = authService.register('contractlist@test.com', 'pass123', 'Test');
  ...
  it('SETTINGS_SET_AI_KEY succeeds when authenticated', () => {
    const user = authService.register('settingskey@test.com', 'pass123', 'Test');
    const userId = getCurrentUserId();
    expect(userId).toBe(user.id);
  ...
  it('requireAuth throws AuthError with descriptive message', () => {
    authService.logout();
    expect(() => authService.requireAuth()).toThrow('Authentication required. Please log in.');
  });
